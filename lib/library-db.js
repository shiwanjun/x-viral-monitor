...
(function (root) {
  ...
  function open() {
    ...
    dbPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        createStore(db, 'relationshipUsers', 'key', [
          ['byAccount', 'accountId'],
          ['byAccountUpdated', ['accountId', 'updatedAt']],
        ]);
        createStore(db, 'relationshipEvents', 'key', [
          ['byAccount', 'accountId'],
          ['byAccountTs', ['accountId', 'ts']],
        ]);
        createStore(db, 'relationshipBackups', 'id', [
          ['byAccount', 'accountId'],
          ['byAccountCreated', ['accountId', 'createdAt']],
        ]);
      };
      request.onsuccess = () => resolve(request.result);
  ...
  function relationshipHandle(value) {
    const handle = cleanText(value, 64).trim().replace(/^@/, '').toLowerCase();
    return /^[a-z0-9_]{1,15}$/.test(handle) ? handle : '';
  }

  function normalizeRelationshipUser(accountId, raw = {}, options = {}) {
    const handle = relationshipHandle(raw.handle || raw.screenName || raw.screen_name);
    if (!handle) throw new Error('invalid_relationship_user');
    const now = Date.now();
    const count = (...values) => {
    ...
    const bool = (...values) => {
      const value = values.find((item) => typeof item === 'boolean' || item === 0 || item === 1);
      return value === undefined ? undefined : Number(value === true || value === 1);
    };
    return {
      key: `${cleanText(accountId, 64)}:${handle}`,
      accountId: cleanText(accountId, 64),
      userId: cleanText(raw.userId || raw.id || raw.id_str, 64),
      handle,
      n: cleanText(raw.n || raw.name, 160),
      a: cleanText(raw.a || raw.avatar || raw.profile_image_url_https, 2_000),
      bio: cleanText(raw.bio || raw.description, 2_000),
      location: cleanText(raw.location, 500),
      url: cleanText(raw.url || raw.expandedUrl, 2_000),
      joinedAt: cleanTimestamp(raw.joinedAt || raw.createdAt || raw.created_at),
      verified: typeof raw.verified === 'boolean' ? raw.verified : undefined,
      blueVerified: typeof raw.blueVerified === 'boolean' ? raw.blueVerified : (typeof raw.is_blue_verified === 'boolean' ? raw.is_blue_verified : undefined),
      verifiedType: cleanText(raw.verifiedType || raw.verificationType, 80),
      protected: typeof raw.protected === 'boolean' ? raw.protected : undefined,
      fc: count(raw.fc, raw.followersCount, raw.followers_count),
      fd: count(raw.fd, raw.followingCount, raw.friends_count),
      statusesCount: count(raw.statusesCount, raw.statuses_count),
      mediaCount: count(raw.mediaCount, raw.media_count),
      favouritesCount: count(raw.favouritesCount, raw.favourites_count),
      listedCount: count(raw.listedCount, raw.listed_count),
      f: bool(raw.f, raw.following),
      b: bool(raw.b, raw.followedBy, raw.followed_by),
      r: count(raw.r, raw.followRate),
      t: cleanNumber(raw.t || raw.lastSeenAt || raw.updatedAt) || now,
      u: cleanNumber(raw.u || raw.unfollowedMeAt) || null,
      i: cleanNumber(raw.i || raw.iUnfollowedAt) || null,
      m: cleanNumber(raw.m || raw.mutualBrokenAt) || null,
      source: cleanText(options.source || raw.source || 'unknown', 80),
      updatedAt: cleanNumber(raw.updatedAt || raw.t) || now,
    };
  }

  function mergeRelationshipUser(current, incoming) {
    if (!current) return incoming;
    const result = { ...current, key: incoming.key, accountId: incoming.accountId, handle: incoming.handle };
    const textFields = ['userId', 'n', 'a', 'bio', 'location', 'url', 'verifiedType'];
    textFields.forEach((key) => { if (incoming[key]) result[key] = incoming[key]; });
    const profileFields = ['joinedAt', 'verified', 'blueVerified', 'protected', 'fc', 'fd', 'statusesCount', 'mediaCount', 'favouritesCount', 'listedCount', 'r'];
    profileFields.forEach((key) => { if (incoming[key] !== undefined) result[key] = incoming[key]; });
    if (Number(incoming.t || 0) >= Number(current.t || 0)) {
      ['f', 'b', 'u', 'i', 'm', 't', 'source'].forEach((key) => {
  ...
  function reconcileRelationshipSnapshot(previousUsers = {}, previousSnap = {}, currentUsers = {}, nextSnap = {}, now = Date.now()) {
    const toMap = (input) => {
      const map = new Map();
      const rows = Array.isArray(input) ? input : Object.entries(input || {}).map(([handle, value]) => ({ handle, ...value }));
      rows.forEach((row) => { const handle = relationshipHandle(row?.handle || row?.h); if (handle) map.set(handle, row); });
      return map;
    };
    const previous = toMap(previousUsers);
    const observed = toMap(currentUsers);
    const oldFollowing = previousSnap?.following || {};
    const oldFollowers = previousSnap?.followers || {};
    const following = nextSnap?.following || {};
    const followers = nextSnap?.followers || {};
    const handles = new Set([
      ...previous.keys(), ...observed.keys(),
      ...Object.keys(oldFollowing), ...Object.keys(oldFollowers),
      ...Object.keys(following), ...Object.keys(followers),
    ].map(relationshipHandle).filter(Boolean));
    const result = {};
    handles.forEach((handle) => {
      const old = previous.get(handle) || {};
      const incoming = observed.get(handle) || {};
      const wasFollowing = Boolean(oldFollowing[handle] || old.f);
      const wasFollower = Boolean(oldFollowers[handle] || old.b);
      const isFollowing = Boolean(following[handle]);
      const isFollower = Boolean(followers[handle]);
      result[handle] = {
        ...old,
        ...Object.fromEntries(Object.entries(incoming).filter(([, value]) => value !== undefined && value !== '')),
        handle, f: isFollowing ? 1 : 0, b: isFollower ? 1 : 0, t: now,
        ...((wasFollower && !isFollower) ? { u: old.u || now } : {}),
        ...((wasFollowing && !isFollowing) ? { i: old.i || now } : {}),
        ...((wasFollowing && wasFollower && !(isFollowing && isFollower)) ? { m: old.m || now } : {}),
      };
    });
    return result;
  }

  async function upsertRelationships(accountId, records, options = {}) {
    const id = cleanText(accountId, 64);
    if (!id) throw new Error('x_account_required');
    const prepared = [];
    const errors = [];
    for (const raw of (Array.isArray(records) ? records : Object.entries(records || {}).map(([handle, value]) => ({ handle, ...value })))) {
      try { prepared.push(normalizeRelationshipUser(id, raw, options)); }
      catch (error) { errors.push(String(error?.message || error)); }
    }
    if (!prepared.length) return { accepted: 0, inserted: 0, updated: 0, errors: errors.slice(0, 10) };
    const db = await open();
    // 页面 store 通常只上报本轮发生变化的少量用户。按 key 读取可以避免
    // 每次滚动时间线都把几千到十万条关系记录完整载入内存。
    const readTx = db.transaction('relationshipUsers', 'readonly');
    const readStore = readTx.objectStore('relationshipUsers');
    const currentRows = await Promise.all(prepared.map((row) => requestResult(readStore.get(row.key))));
    const current = new Map(currentRows.filter(Boolean).map((row) => [row.handle, row]));
    const tx = db.transaction('relationshipUsers', 'readwrite');
    let inserted = 0; let updated = 0;
    for (const incoming of prepared) {
      const existing = current.get(incoming.handle);
      tx.objectStore('relationshipUsers').put(mergeRelationshipUser(existing, incoming));
      if (existing) updated += 1; else inserted += 1;
    }
    await transactionDone(tx);
    return { accepted: prepared.length, inserted, updated, errors: errors.slice(0, 10) };
  }

  function normalizeRelationshipEvent(accountId, raw = {}) {
    const ts = cleanNumber(raw.ts || raw.updatedAt) || Date.now();
    const handle = relationshipHandle(raw.h || raw.handle);
    const type = cleanText(raw.type, 80);
    const id = cleanText(raw.id || `${type}:${handle}:${ts}`, 240);
    if (!id || !handle || !type) throw new Error('invalid_relationship_event');
    return {
      key: `${cleanText(accountId, 64)}:${id}`,
      id,
      accountId: cleanText(accountId, 64),
      h: handle,
      n: cleanText(raw.n || raw.name, 160),
      a: cleanText(raw.a || raw.avatar, 2_000),
      type,
      ts,
      fc: hasValue(raw.fc) && Number.isFinite(Number(raw.fc)) ? Number(raw.fc) : undefined,
      fd: hasValue(raw.fd) && Number.isFinite(Number(raw.fd)) ? Number(raw.fd) : undefined,
      updatedAt: cleanNumber(raw.updatedAt) || ts,
    };
  }

  async function deleteRelationshipRows(accountId) {
    const db = await open();
    const stores = ['relationshipUsers', 'relationshipEvents'];
    for (const storeName of stores) {
      const rows = await requestResult(db.transaction(storeName, 'readonly').objectStore(storeName).index('byAccount').getAll(accountId));
      if (!rows.length) continue;
      const tx = db.transaction(storeName, 'readwrite');
      rows.forEach((row) => tx.objectStore(storeName).delete(row.key));
      await transactionDone(tx);
    }
  }

  async function putRelationshipEvents(accountId, events) {
    const prepared = [];
    for (const raw of (Array.isArray(events) ? events : [])) {
      try { prepared.push(normalizeRelationshipEvent(accountId, raw)); } catch (_) {}
    }
    if (!prepared.length) return 0;
    const db = await open();
    const tx = db.transaction('relationshipEvents', 'readwrite');
    prepared.forEach((event) => tx.objectStore('relationshipEvents').put(event));
    await transactionDone(tx);
    return prepared.length;
  }

  async function createRelationshipBackup(accountId, label = '自动备份') {
    const current = await getRelationships(accountId);
    if (!current.users.length && !current.events.length) return null;
    const db = await open();
    const createdAt = Date.now();
    const id = `relationship-backup:${cleanText(accountId, 64)}:${createdAt}:${Math.random().toString(36).slice(2, 8)}`;
    const record = {
      id, accountId: cleanText(accountId, 64), label: cleanText(label, 120), createdAt,
      data: { users: current.users, events: current.events, snap: current.snap, meta: current.meta },
    };
    const tx = db.transaction('relationshipBackups', 'readwrite');
    tx.objectStore('relationshipBackups').put(record);
    await transactionDone(tx);
    const backups = await requestResult(db.transaction('relationshipBackups', 'readonly').objectStore('relationshipBackups').index('byAccount').getAll(accountId));
    backups.sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
    if (backups.length > 5) {
      const prune = db.transaction('relationshipBackups', 'readwrite');
      backups.slice(5).forEach((backup) => prune.objectStore('relationshipBackups').delete(backup.id));
      await transactionDone(prune);
    }
    return record;
  }

  async function putRelationshipSnapshot(accountId, payload = {}, options = {}) {
    const id = cleanText(accountId, 64);
    if (!id) throw new Error('x_account_required');
    let backup = null;
    if (options.replace && !options.skipBackup) backup = await createRelationshipBackup(id, options.backupLabel || '关系快照替换前');
    if (options.replace) await deleteRelationshipRows(id);
    const result = await upsertRelationships(id, payload.users || [], { source: options.source || payload.meta?.source || 'snapshot' });
    const events = await putRelationshipEvents(id, payload.events || []);
    const previousState = options.replace ? null : await getSyncState(`relationships:${id}`);
    await setSyncState(`relationships:${id}`, {
      accountId: id,
      snap: payload.snap || previousState?.snap || null,
      meta: { ...(previousState?.meta || {}), ...(payload.meta || {}), storage: 'indexeddb' },
      updatedAt: Date.now(),
    });
    return { ...result, events, backupId: backup?.id || null };
  }

  async function getRelationships(accountId) {
    const id = cleanText(accountId, 64);
    if (!id) throw new Error('x_account_required');
    const db = await open();
    const [users, events, state] = await Promise.all([
      requestResult(db.transaction('relationshipUsers', 'readonly').objectStore('relationshipUsers').index('byAccount').getAll(id)),
      requestResult(db.transaction('relationshipEvents', 'readonly').objectStore('relationshipEvents').index('byAccount').getAll(id)),
      getSyncState(`relationships:${id}`),
    ]);
    users.sort((a, b) => Number(b.t || b.updatedAt || 0) - Number(a.t || a.updatedAt || 0));
    events.sort((a, b) => Number(b.ts || 0) - Number(a.ts || 0));
    return {
      accountId: id, users, events,
      snap: state?.snap || null,
      meta: { ...(state?.meta || {}), storage: 'indexeddb', count: users.length },
    };
  }

  async function listRelationshipBackups(accountId) {
    const db = await open();
    const rows = await requestResult(db.transaction('relationshipBackups', 'readonly').objectStore('relationshipBackups').index('byAccount').getAll(cleanText(accountId, 64)));
    return rows.sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0)).map(({ data, ...summary }) => ({ ...summary, users: data?.users?.length || 0, events: data?.events?.length || 0 }));
  }

  async function exportRelationshipBackup(accountId) {
    const current = await getRelationships(accountId);
    return {
      schema: 'x-tools-relationships', version: 1,
      accountId: current.accountId, exportedAt: Date.now(),
      data: { users: current.users, events: current.events, snap: current.snap, meta: current.meta },
    };
  }

  async function importRelationshipBackup(accountId, backup, options = {}) {
    if (backup?.schema !== 'x-tools-relationships' || Number(backup?.version) !== 1 || !backup?.data) throw new Error('invalid_relationship_backup');
    if (backup.accountId && String(backup.accountId) !== String(accountId)) throw new Error('account_mismatch');
    return putRelationshipSnapshot(accountId, backup.data, {
      replace: options.replace !== false,
      backupLabel: options.backupLabel || '导入备份前',
      source: 'backup_restore',
    });
  }

  async function restoreRelationshipBackup(accountId, backupId) {
    const db = await open();
    const backup = await requestResult(db.transaction('relationshipBackups', 'readonly').objectStore('relationshipBackups').get(cleanText(backupId, 260)));
    if (!backup || backup.accountId !== String(accountId)) throw new Error('relationship_backup_not_found');
    return putRelationshipSnapshot(accountId, backup.data, {
      replace: true,
      backupLabel: '回滚前自动备份',
      source: 'rollback',
  ...
  root.XvmLibraryDb = {
    ...
    archive, listArchived, getSyncState, setSyncState, markSourceRemoved, validateXAction, readOutbox, ackOutbox, applyChanges,
    normalizeRelationshipUser, mergeRelationshipUser, reconcileRelationshipSnapshot, upsertRelationships, putRelationshipSnapshot, getRelationships,
    createRelationshipBackup, listRelationshipBackups, exportRelationshipBackup, importRelationshipBackup, restoreRelationshipBackup,
    clearAll, close,
  };
