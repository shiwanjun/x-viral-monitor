...
(() => {
  ...
  const READ_OPERATIONS = new Set([
    'Bookmarks', 'BookmarkFoldersSlice', 'BookmarkFolderTimeline',
    'Likes', 'UserTweets', 'UserTweetsAndReplies', 'Following', 'Followers',
  ]);
  ...
  function liveHeaders(template, extra = {}) {
    const csrf = document.cookie.match(/(?:^|;\s*)ct0=([^;]+)/)?.[1] || '';
    const bearer = window.__xvmNet?.getBearer?.() || '';
    return {
      ...(template?.headers || {}),
  ...
  function emitAuth() {
    const csrf = document.cookie.match(/(?:^|;\s*)ct0=([^;]+)/)?.[1] || '';
    const authorization = window.__xvmNet?.getBearer?.() || '';
    const id = accountId();
    if (!id || !csrf || !authorization) return false;
    emit('XVM_LIBRARY_AUTH', {
      accountId: id,
      auth: { authorization, csrfToken: decodeURIComponent(csrf), capturedAt: Date.now() },
    });
    return true;
  }
  ...
  async function captureResponse(rawUrl, response) {
    ...
    if (READ_OPERATIONS.has(parsed.operation)) {
      ...
      const records = window.XvmLibraryNormalize.findTweets(json).map((tweet) => window.XvmLibraryNormalize.normalizeTweet(tweet, window.XvmLibraryNormalize.kindForOperation(parsed.operation), viewerId)).filter(Boolean);
      emit('XVM_LIBRARY_CAPTURE_BATCH', { accountId: viewerId, operation: parsed.operation, records, cursor: window.XvmLibraryNormalize.cursorFrom(json) });
      return;
    }
  ...
  function buildReplay(template, cursor) {
    ...
    return { url: url.href, init: { method: 'GET', credentials: 'include', headers: liveHeaders(template) } };
  }
  ...
  async function syncOperation(operation, mode = 'incremental') {
    ...
    const resume = mode === 'full' ? (template.resumeCursor || '') : '';
    const highWaterId = mode === 'incremental' ? String(template.highWaterId || '') : '';
    let cursor = resume, pages = 0, captured = 0, backoff = 2500;
    activeSyncs.set(operation, true);
    while (activeSyncs.get(operation) && pages < maxPages) {
      const request = buildReplay(template, cursor);
      const replayFetch = window.__xvmNet?.originalFetch || window.fetch.bind(window);
      const response = await replayFetch(request.url, request.init);
      ...
      const viewerId = accountId();
      const records = window.XvmLibraryNormalize.findTweets(json).map((tweet) => window.XvmLibraryNormalize.normalizeTweet(tweet, window.XvmLibraryNormalize.kindForOperation(operation), viewerId)).filter(Boolean);
      const reachedHighWater = Boolean(highWaterId && records.some((record) => record.post.id === highWaterId));
      captured += records.length; pages += 1;
      cursor = window.XvmLibraryNormalize.cursorFrom(json);
      emit('XVM_LIBRARY_CAPTURE_BATCH', { accountId: viewerId, operation, records, cursor });
      emit('XVM_LIBRARY_SYNC_PROGRESS', {
        operation,
        status: cursor && !reachedHighWater ? 'running' : 'done',
        pages,
        captured,
        cursor,
        highWaterId: pages === 1 && !resume ? (records[0]?.post?.id || highWaterId) : '',
        reachedHighWater,
      });
  ...
  async function runXAction(action) {
    ...
    for (const postId of ids) {
      try {
        ...
        const replayFetch = window.__xvmNet?.originalFetch || window.fetch.bind(window);
        const response = await replayFetch(url.href, { method: 'POST', credentials: 'include', headers: liveHeaders(template, { 'content-type': 'application/json' }), body });
        results.push({ postId, ok: response.ok, status: response.status });
        if (response.ok) emit('XVM_LIBRARY_SOURCE_REMOVED', { accountId: accountId(), kind: WRITE_KIND[operation], postId: String(postId) });
  ...
  async function runAiClassify(request) {
    if (!window.__xvmGrok?.generate) throw new Error('grok_unavailable');
    const text = String(request?.text || '').slice(0, 12_000);
    if (!text) throw new Error('invalid_request');
    const comments = await window.__xvmGrok.generate({
      tweetText: text,
      promptTemplate: '[推文内容]\n\n请提炼 3-8 个可复用的中文内容标签。每个标签 2-10 个字，每个标签单独放进一个代码块，不要编号，不要解释。',
      temporaryChat: true,
    });
    emit('XVM_LIBRARY_AI_RESULT', { requestId: request.requestId, comments });
  }
  ...
  window.addEventListener('message', (event) => {
    ...
    if (event.data.type === 'XVM_LIBRARY_AI_COMMAND') runAiClassify(event.data.request).catch((error) => emit('XVM_LIBRARY_AI_RESULT', { requestId: event.data.request?.requestId, error: String(error?.message || error) }));
  });
