(() => {
  ...
  function sanitizeTweetIds(tweetIds, limit = MAX_SEEN_TWEET_IDS) {
    return [...new Set((Array.isArray(tweetIds) ? tweetIds : [])
      .map((value) => String(value || '').trim())
      .filter((value) => /^\d{1,32}$/.test(value)))]
      .slice(-Math.max(1, Number.parseInt(limit, 10) || MAX_SEEN_TWEET_IDS));
  }
  ...
  function mergeSeenTweetIds(raw, accountId, tweetIds, limit = MAX_SEEN_TWEET_IDS) {
    const id = String(accountId || '');
    if (!id) return { accountId: '', ids: [] };
    const existing = raw?.accountId === id ? raw.ids : [];
    return { accountId: id, ids: sanitizeTweetIds([...sanitizeTweetIds(existing), ...sanitizeTweetIds(tweetIds)], limit) };
  }
