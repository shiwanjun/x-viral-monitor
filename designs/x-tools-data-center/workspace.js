(() => {
  ...
  async function importRelationshipBackup(file) {
    ...
    catch (_) { toast('备份文件格式无效或内容损坏'); return; }
  ...
  async function rollbackRelationshipBackup() {
    ...
    if (!latest) return toast('没有可回滚的本地关系快照');
  ...
  async function sync(mode, kind = state.kind) {
    ...
    if (result.ok) {
      ...
      const poll = async () => {
        ...
        if (!['running', 'rate_limited'].includes(state.sync.status) || attempts >= 100) { clearSyncPoll(); await query(true); return; }
