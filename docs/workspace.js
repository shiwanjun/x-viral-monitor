(() => {
  ...
  function clearSyncPoll() { if (syncPollTimer) clearTimeout(syncPollTimer); syncPollTimer = 0; }
  ...
  async function refreshStatus() {
    ...
    const active = ['running', 'rate_limited', 'network_retry', 'http_retry'].includes(result.sync?.status);
    const operation = result.sync?.currentOperation ? ` · ${result.sync.currentOperation}` : '';
    banner(result.sync?.status === 'rate_limited' || failed ? 'error' : 'ok', active ? '正在同步' : failed ? '同步未完成' : '扩展已连接', result.sync?.status === 'rate_limited' ? `X 已限流，约 ${Math.ceil(Number(result.sync.retryMs || 0) / 1000)} 秒后自动继续${operation}` : active ? `已逐页保存，可安全离开页面${operation}` : failed ? errorText(result.sync?.error) : `本地数据库可用 · ${result.account?.accountId ? `已绑定 X ID ${result.account.accountId}` : '等待绑定 X 账号'}`, active ? '暂停' : '刷新');
    renderAccount();
  ...
  async function query(reset = false) {
    ...
    if (sequence !== querySequence || requestedKind !== state.kind) return;
  ...
  function renderEmpty(disconnected = false) { const empty = $('#empty'); const hasRows = state.rows.length > 0; const table = $('#view-table'); empty.hidden = hasRows || state.view === 'stats'; table.hidden = state.view !== 'table'; table.classList.toggle('has-rows', hasRows); $('#view-gallery').hidden = disconnected || !hasRows || state.view !== 'gallery'; $('#view-stats').hidden = disconnected || state.view !== 'stats'; }
  ...
  function relationshipKind(user) { return user.f && user.b ? 'mutual' : user.m ? 'unfollowed' : user.f ? 'mine' : user.b ? 'theirs' : (user.u || user.i) ? 'unfollowed' : 'none'; }
  function relationshipLabel(kind) { return ({ mutual: '互关', mine: '我关注了', theirs: '关注我', unfollowed: '取关了', none: '关注率' })[kind] || kind; }
  ...
  async function sync(mode, kind = state.kind) {
    ...
    if (result.ok) {
      let attempts = 0;
      const poll = async () => {
        attempts += 1; await refreshStatus();
        if (!['running', 'rate_limited', 'network_retry', 'http_retry'].includes(state.sync.status) || attempts >= 600) { clearSyncPoll(); await query(true); return; }
        syncPollTimer = setTimeout(poll, 1200);
      };
      syncPollTimer = setTimeout(poll, 1200);
    }
  }
  ...
  async function syncRelationships(section) {
    ...
    toast(result.ok ? '关注关系扫描已开始，旧快照会保持到完整扫描成功' : errorText(result.error));
    if (!result.ok) { button?.classList.remove('syncing'); return; }
    let attempts = 0;
    const poll = async () => {
      attempts += 1;
      await refreshRelationships();
      const status = state.relations.sync?.status;
      if (!['running', 'rate_limited'].includes(status) || attempts >= 600) {
        button?.classList.remove('syncing');
        toast(status === 'failed' ? errorText(state.relations.sync?.error) : '关注关系快照已更新');
        return;
      }
  ...
  function openPostByItemId(itemId) { const row = state.rows.find(({ item }) => item.id === itemId); if (row) window.open(`https://x.com/${encodeURIComponent(row.post.authorHandle)}/status/${encodeURIComponent(row.post.id)}`, '_blank', 'noopener'); }
  function openUser(handle) { if (handle) window.open(`https://x.com/${encodeURIComponent(handle)}`, '_blank', 'noopener'); }
  function activateLinkedRow(event) { const interactive = event.target.closest('input,button,a,select,label'); if (interactive) return; const post = event.target.closest('[data-open-post]')?.dataset.openPost; const user = event.target.closest('[data-open-user]')?.dataset.openUser; if (post) openPostByItemId(post); else if (user) openUser(user); }
  ...
  $('#kind-nav').addEventListener('click', (event) => { const syncButton = event.target.closest('[data-sync-kind]'); if (syncButton) { event.stopPropagation(); sync('incremental', syncButton.dataset.syncKind); return; } const button = event.target.closest('[data-kind]'); if (!button) return; state.kind = button.dataset.kind; state.selected.clear(); $$('#kind-nav [data-kind]').forEach((item) => item.classList.toggle('active', item === button)); $('#page-title').textContent = kindName[state.kind]; showSection('library'); query(true); });
  $('#kind-nav').addEventListener('click', (event) => { const syncButton = event.target.closest('[data-sync-section]'); if (syncButton) { event.stopPropagation(); syncRelationships(syncButton.dataset.syncSection); return; } const button = event.target.closest('[data-section]'); if (button) showSection(button.dataset.section); });
  ...
  $('#rows').addEventListener('click', activateLinkedRow); $('#view-gallery').addEventListener('click', activateLinkedRow); $('#relation-list').addEventListener('click', activateLinkedRow); $('#unfollow-list').addEventListener('click', activateLinkedRow);
  document.addEventListener('keydown', (event) => { if (!['Enter', ' '].includes(event.key)) return; const row = event.target.closest('[data-open-post],[data-open-user]'); if (!row) return; event.preventDefault(); activateLinkedRow({ target: row }); });
